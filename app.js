...
function renderBooks() {
  ...
  bookGrid.innerHTML = matchingBooks.map((book, index) => `<article class="book-card fade-in" data-book="${book.id}" style="animation-delay:${index * 80}ms"><div class="cover ${book.color}"><span class="cover-meta">阅途 / ${String(index + 1).padStart(2,'0')}</span><span class="cover-title">${book.coverTitle.replace(/\n/g, '<br>')}</span><span class="cover-meta">${book.tag.split(' / ')[0]}</span></div><div class="book-info"><h3>${book.title}</h3><p>${book.author}</p><strong class="book-price">${book.price.toFixed(2)} ${book.currency}</strong><span class="book-tag">${book.tag}</span><button class="add-to-cart" type="button" data-add-book="${book.id}">加入购物车 <span aria-hidden="true">+</span></button></div></article>`).join('');
